...
import { DeleteResponse, FAQ, FAQCategories } from '@models/index';
...
export class FAQService {
  ...
  categoriesSubject: BehaviorSubject<FAQCategories[]> = new BehaviorSubject<
    FAQCategories[]
  >(this.categories);
  ...
  fetchFAQ(id: string): Promise<FAQ> {
    return this.http
      .get<FAQ>(`http://localhost:3000/v1/faq/${id}`, {
  ...
  fetchCategories(): Promise<FAQCategories[]> {
    return this.http
      .get<FAQCategories[]>(`http://localhost:3000/v1/faq-categories`, {
        headers: new HttpHeaders({
          'Content-Type': 'application/json',
          Authorization: getFullToken(),
        }),
  ...
  saveFAQ(data: FAQ): Promise<FAQ> {
    return this.http
      .post<FAQ>(`http://localhost:3000/v1/faq`, data, {
        headers: new HttpHeaders({
          'Content-Type': 'application/json',
          Authorization: getFullToken(),
        }),
  ...
  deleteFAQ(id: string): Promise<FAQ> {
    return this.http
      .request<FAQ>('delete', `http://localhost:3000/v1/faq/${id}`, {
        headers: new HttpHeaders({
          'Content-Type': 'application/json',
          Authorization: getFullToken(),
        }),
  ...
  deleteFAQs(): Promise<DeleteResponse> {
    return this.http
      .request<DeleteResponse>('delete', `http://localhost:3000/v1/faq`, {
        headers: new HttpHeaders({
          'Content-Type': 'application/json',
          Authorization: getFullToken(),
        }),
  ...
  updateFAQ(id: string, data: FAQ): Promise<FAQ> {
    return this.http
      .put<FAQ>(`http://localhost:3000/v1/faq/${id}`, data, {
        headers: new HttpHeaders({
          'Content-Type': 'application/json',
          Authorization: getFullToken(),
        }),
